...
func writeMFD(p1, p2 *MapsforgeParser, outputPath string) error {
	...
	// Compact header: h2's structural fields with empty tag tables.
	// Tag tables are stored separately as a diff below.
	h2compact := *h2
	h2compact.poi_tags = nil
	h2compact.way_tags = nil
	headerBytes := serializeMapforgeHeader(&h2compact)
	...
	// Sort records by (si, y, x) to maximise delta-coordinate savings.
	...
	// Coordinates are delta-encoded: dy=Δy within same si (absolute for first record per si),
	// dx=Δx within same row (absolute for first record in a new row or new si).
	rRec := newRawWriter()
	prevSI := -1
	prevX := 0
	prevY := 0
	for _, rec := range records {
		...
		if rec.si != prevSI {
			...
			dy = uint32(rec.y - prevY)
			dx = uint32(rec.x)
		} else {
			dy = 0
			dx = uint32(rec.x - prevX)
		}
		rRec.VbeU(dy)
		rRec.VbeU(dx)
		prevSI = rec.si
	...
	// Tag diff: encode new tag list as references into old list (retained tags)
	// or inline strings (new tags). Eliminates the separate poiMap/wayMap section.
	writeTagDiff(w, h1.poi_tags, h2.poi_tags)
	writeTagDiff(w, h1.way_tags, h2.way_tags)
...
// writeTagDiff encodes the new tag list as a compact diff against the old tag list.
// Each entry is a VbeU: 0 = followed by a new VbeString literal; n>0 = oldTags[n-1].
func writeTagDiff(w *raw_writer, oldTags, newTags []string) {
	oldIdx := make(map[string]uint32, len(oldTags))
	for i, t := range oldTags {
		oldIdx[t] = uint32(i)
	}
	w.VbeU(uint32(len(newTags)))
	for _, tag := range newTags {
		if idx, ok := oldIdx[tag]; ok {
			w.VbeU(idx + 1) // 1-indexed back-reference
		} else {
			w.VbeU(0) // literal new tag follows
			w.VbeString(tag)
		}
	}
}
...
func computeDeltaRecord(p1, p2 *MapsforgeParser, si, x, y int, poiMap, wayMap []uint32, mw *MapsforgeWriter, idx *lz77Index) (*mfdTileRecord, error) {
	...
	if b1 != nil && isWater1 == isWater2 && isIdentityMapping(poiMap) && isIdentityMapping(wayMap) && bytes.Equal(b1, b2) {
		return nil, nil
	}

	zic := &p2.data.header.zoom_interval[si]

	// Pre-compute p1's zoom interval for this subfile (needed for both the
	// streaming fast-path guard and the full-parse zoom-index mapping below).
	var p1Zic *ZoomIntervalConfig
	if si < len(p1.data.header.zoom_interval) {
		p1Zic = &p1.data.header.zoom_interval[si]
	}
	...
	// Only valid when p1 and p2 share the same zoom range: streamTilesEqual reads
	// b1 using p2's zic, so a different zoom structure produces wrong results.
	sameZoomRange := p1Zic != nil &&
		p1Zic.min_zoom_level == zic.min_zoom_level &&
		p1Zic.max_zoom_level == zic.max_zoom_level
	if b1 != nil && isWater1 == isWater2 && sameZoomRange && streamTilesEqual(b1, b2, poiMap, wayMap, &p1.data.header, zic) {
		return nil, nil
	}
	zooms := int(zic.max_zoom_level-zic.min_zoom_level) + 1
	...
	for zi := 0; zi < zooms; zi++ {
		...
		var refBlob []byte
		if td1 != nil {
			// Map p2 zoom index to p1 zoom index via actual zoom level.
			var remappedPOIs []POIData
			var remappedWays []WayProperties
			if p1Zic != nil {
				zi1 := int(zic.min_zoom_level) + zi - int(p1Zic.min_zoom_level)
				if zi1 >= 0 && zi1 < len(td1.poi_data) {
					remappedPOIs = remapPOITags(td1.poi_data[zi1], poiMap)
					remappedWays = remapWayTags(td1.way_data[zi1], wayMap)
				}
			}
