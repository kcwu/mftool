...
Magic: `"mfd\x05"` (4 bytes).

```
[magic 4B]
[header_len VbeU][header_bytes]        -- compact Mapsforge header of the NEW map
                                          (all structural fields; tag tables are EMPTY)
[poi_tag_count VbeU]                   -- tag diff for POI tags (see Tag Diff below)
  per entry: [ref VbeU]
    ref == 0: [new_tag VbeString]      -- literal new tag (not in old map)
    ref  > 0: back-reference to prevTags[ref-1]
[way_tag_count VbeU]                   -- tag diff for way tags (same encoding)
  per entry: [ref VbeU] ...
[records_zstd_len VbeU][records_zstd_bytes]  -- zstd-compressed records section
  Records are sorted by (si, y, x) before encoding.
  per changed tile:
    [si uint8]                         -- subfile index (0–254; 0xFF = end-of-stream sentinel)
    [dy VbeU][dx VbeU]                 -- delta-encoded tile coordinates (see below)
...
Records are sorted by `(si, y, x)`. Coordinates are stored as non-negative deltas to exploit the sequential ordering:

| Condition | dy | dx |
|---|---|---|
| First record for a new `si` | absolute `y` | absolute `x` |
| Same `si`, new row (`y` changed) | `y − prev_y` | absolute `x` |
| Same `si`, same row | `0` | `x − prev_x` |

All values are non-negative, so `VbeU` (unsigned) is sufficient. Most records within a subfile share `dy=0` and have a small `dx`, saving 1–2 bytes per record versus absolute coordinates. Measured across an 8-delta chain (~368 MB map, ~2600 changed tiles/delta): saves 759–4779 bytes per delta (~2 KB average) after zstd. The sorting also regularises the record byte stream, which zstd exploits — savings after zstd are comparable to the raw savings. Total across 8 deltas: ~16.6 KB (~0.6%). No measurable impact on delta generation or apply performance.

### Tag Diff

Each new tag list is stored as a compact diff against the *previous version*'s tag list rather than as a full string table.

Each entry is a VbeU:
- `0` — new tag; the tag string follows as a VbeString.
- `n > 0` — retained tag; `prevTags[n-1]` is the tag string (1-indexed back-reference).

`loadMFD` reconstructs the full new tag list from the diff, then derives `poiMap`/`wayMap` via `buildTagMapByString(prevTags, newTags)`.

Because each delta's tag tables depend on the previous version's, `loadMFD` takes `prevPOITags`/`prevWayTags` as parameters. `CmdApply` passes them sequentially: base → delta 0 → delta 1 → ….

**Size**: the diff typically encodes to ~1.2 KB per delta file. Impact is largest for small deltas — a 614 KB delta is ~2% tag diff; a 719 KB delta is ~0.2%.
...
[poi_bytes_len VbeU]
[poi_bytes]       -- serialized POIData records (tile-relative VbeS lat/lon, tags, name, ...)
[way_bytes]       -- serialized WayProperties records (VbeS delta-encoded coordinates, tags, ...)
```
...
- Total MFD size: ~700–720 KB for a 368 MB map (varies by delta; measured range across 8 consecutive weekly deltas: 614–719 KB).
...
h2compact = new.header; h2compact.poi_tags = nil; h2compact.way_tags = nil
serializeMapforgeHeader(&h2compact)             → compactHeaderBytes
writeTagDiff(old.poi_tags, new.poi_tags)        → poiTagDiffBytes
writeTagDiff(old.way_tags, new.way_tags)        → wayTagDiffBytes
collectDeltaRecords(old, new, poiMap, wayMap)   → []*mfdTileRecord
sort records by (si, y, x)
writeMFD(...)
```

### `collectDeltaRecords` — parallel worker pool

Tiles are dispatched via a buffered channel to `runtime.NumCPU()` workers, each calling `computeDeltaRecord`. A lookahead queue (`[]chan deltaResult`) accumulates pending results; it is drained by one entry whenever its length reaches 4×workers, bounding memory while preserving dispatch order. Results are collected in dispatch order to enable sequential writing.
...
2. **Streaming fast path** (`streamTilesEqual`): only attempted when `b1 != nil`, water flags are equal, and both maps share the same zoom range for this subfile (`sameZoomRange`). Compares tiles field-by-field applying tag remapping inline, with zero allocations. Catches semantically-unchanged tiles even when byte representations differ due to tag ID renumbering. Conservative: returns false if element order differs, falling through to full parse.
3. Parse td2 with `GetTileDataUncached` (always); parse td1 only if the tile exists in the old map (`b1 != nil`). No caching avoids GC pressure on single-use tiles.
4. For each zoom level `zi`:
   - Build **reference** (if td1 != nil): map `zi` to `zi1` (p2's zoom index → p1's zoom index via actual zoom level), then `remapPOITags(td1.poi_data[zi1], poiMap)` + `remapWayTags(...)` → `normalizeZoomLevel(...)` → `encodeZoomBlob(...)`. If td1 is nil, refBlob is empty.
   ...
   - Otherwise: `idx.encode(targetBlob, refBlob)` → patch (method on `lz77Index`). Set bit `zi` in `zoomMask`.
5. Earlier in the function, explicit empty-tile returns handle the `b2 == nil` cases. After full parse: if `zoomMask == 0 && isWater1 == isWater2`: return nil (no record needed).
...
```
ParseFile(base)
prevPOI, prevWay = base.header.poi_tags, base.header.way_tags
for each delta file:
    loadMFD(path, prevPOI, prevWay) → delta
    prevPOI, prevWay = delta.header.poi_tags, delta.header.way_tags
...
Takes `prevPOITags`/`prevWayTags` (from the base map or the preceding delta). Reads the compact header, reconstructs the full tag tables from the tag diff, derives `poiMap`/`wayMap` via `buildTagMapByString`, zstd-decompresses the records section, then parses tile records into `map[mfdTileKey]*mfdTileRecord`.
...
Records within a single delta are independent, so processing is **parallelised** with `runtime.NumCPU()` workers. The full set of `(key, rec, st)` inputs is snapshotted from `tileStates` before any jobs are dispatched (avoiding concurrent map reads and writes). Jobs are sent to a buffered channel; results are returned via a fully-buffered results channel (capacity = number of jobs). The main goroutine collects results and writes them back to `tileStates` — only the main goroutine ever writes to `tileStates`, making it race-free without waiting for all workers to finish first. Unlike `collectDeltaRecords`, no ordering guarantee is needed — `tileStates` is a map, not a sequential stream.
...
First, check if the output and base share the same debug mode and zoom structure (`sameDebug && sameZooms`). If not — e.g., the output has a different zoom range or debug flag — fall back to a full re-encode: `GetTileDataUncachedLight` + tag-remap + `WriteTileData`.

When `sameDebug && sameZooms`:
...
Fast path (condition: `!blobsNeedRemap && !outHasDebug && (allBlobsPresent || !base.data.header.has_debug)`):
...
- The `!outHasDebug` guard is required because zoom blobs are always debug-free; when the output has `has_debug=true`, per-element signatures must be emitted via `WriteTileData` (slow path). The `!base.data.header.has_debug` guard (when not all blobs are present) prevents verbatim use of base debug bytes in a non-debug tile.

Slow path (when any fast-path condition is unmet — `blobsNeedRemap`, `outHasDebug`, or a debug-mode mismatch on base): full struct decode (`decodeZoomBlob`), remap, `WriteTileData`. In multi-delta chains, `blobsNeedRemap` occurs when a zoom level's blob is from an older delta namespace.

### Chain correctness

Each zoom level tracks its own namespace version (`zoomVers[zi]`) independently. When a tile is updated by delta A for zoom levels 0–2 and later by delta B for only zoom level 0, zoom levels 1–2 retain `zoomVers[zi]` pointing to delta A's namespace while zoom level 0 is updated to delta B's namespace. `buildOutputTile` remaps each zoom level individually, preventing the wrong namespace from being applied.
...
- **Apply (8-delta chain)**: ~1.6s wall on an 8-core machine.
  ...
  - GC pressure reduced by `debug.SetGCPercent(400)` at the start of both `CmdApply` and `CmdDelta`, and by deferring struct allocations to the rare slow path.
